...
class AttendanceAdmin {
    ...
    generateQRCode() {
        ...
        // Generate time-based rotation code (changes every 3 minutes)
        const now = new Date();
        const timeSlot = Math.floor(now.getTime() / (3 * 60 * 1000)); // 3-minute intervals
        ...
        const nextRotation = new Date((timeSlot + 1) * 3 * 60 * 1000);
        expirySpan.textContent = `Next code: ${nextRotation.toLocaleTimeString()}`;
        
        qrDisplay.style.display = 'block';
        
        // Auto-refresh QR code every 3 minutes
